...
def append_to_csv(data:dict, path_to_file: str, **kwargs):
    ...
    if os.path.exists(path_to_file) and os.stat(path_to_file).st_size == 0 or not os.path.exists(path_to_file):
        pd.DataFrame.from_dict(data, orient='columns').to_csv(path_to_file, **kwargs)
        return
    pd.DataFrame.from_dict(data, orient='columns').to_csv(path_to_file, header=False,  **kwargs)
        ...
        for i in range(1, 10):
            ...
            for name, fun in list_of_functions.items():
                if i == 1:
                    ...
                    print("a")
                    append_to_csv(dictionary, f'./csv_output/fp.csv', index=[0],  mode='a+', sep=';')
                ...
                append_to_csv(dictionary, f'./csv_output/{i}key_misspells.csv',  index=True,mode='a+', sep=';')
                ...
                append_to_csv(dictionary, f'./csv_output/{i}rand_misspells.csv', index=True, mode='a+', sep=';')
